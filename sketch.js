...
var waves;
var avancar, vmenu, feliz, triste, conseguiu, derrota1, novaTentativa, fase2, fase3;
var pno, flt, viol;//imagens
var mainmenu, parabens, trilha2;//músicas
var ac_violao, pianoSound, fluteSound;//instrumentos
...
function preload() {
  ...
  avancar = loadImage('archives pctures/avançar.png')//avançar
  vmenu = loadImage('archives pctures/v_menu.png')// voltar ao menu
  novaTentativa = loadImage('archives pctures/novamente.png')// tentar denovo
  ...
  ac_violao = loadSound('Audios/ac_guit.mp3')
  pianoSound = loadSound('Audios/piano.mp3')
  fluteSound = loadSound('Audios/flute.mp3')
  //sons do jogo
  parabens = loadSound('Audios/Teclado/Parabéns.mp3')
  trilha2 = loadSound('Audios/trilha2.mp3')
  //outros sons
  mainmenu = loadSound('Audios/mainMenu.mp3')
  ...
  fase2 = loadImage('archives pctures/fase2.png')
  fase3 = loadImage('archives pctures/fase3.png')
  //outras imagens
...
function draw() {
  ...
  if(tela==5){ // tela da segunda fase do jogo
    tela21()
  }
  if(tela==7){ // tela da vitória
    tela4()
  }
  if(tela==8){ // tela do fracasso
    tela12()
  }
  if(tela==18){
    mudançaDeFase()
...
function tela12(){//tela do fracasso
  ...
  if(mouseX>1200 && mouseX<1260 && mouseY>50 && mouseY<110){//volta ao menu
    rect(1190, 45, 80, 75, 5)
    if(mouseIsPressed){
      erro.stop()
      tela=0
      mainmenu.play()
  ...
  image(vmenu, 1200, 50, 60, 60)//menu
  if(mouseX>windowWidth/2-100 && mouseX<windowWidth/2+100 && mouseY>windowHeight/2-50 && mouseY<windowHeight-30){
    rect(windowWidth/2-110, windowHeight/2-55, 220, 30, 5)
    if(mouseIsPressed){
      erro.stop()
      tela=4
    }
  }
  image(novaTentativa, windowWidth/2-100, windowHeight/2-50, 200, 20)
}
...
function mudançaDeFase(){
  ...
  textSize(60)
  textAlign(CENTER)
  text("Acertou todas", windowWidth/2, 280)
  text("obrigado por jogar", windowWidth/2, 360)
}

function tela21(){// fase 2
  image(fase2, windowWidth/2-20, 20, 100, 34)
  image(volta, 1200, 60, 80, 60)// botão de voltar ao menu
  //image(tgame, windowWidth/2-150, 30, 300, 100)
  if(mouseX>1200 && mouseX<1280 && mouseY>60 && mouseY<120){
    if(mouseIsPressed){
  ...
  if(mouseX>350 && mouseX<390 && mouseY>450 && mouseY<490){//2
    rect(330, 430, 80, 80, 5)
    if(mouseIsPressed){
      trilha2.stop()
      aplausos.play()
      tela=18 // tela da vitória
    }
  }
  if(mouseX>550 && mouseX<590 && mouseY>450 && mouseY<490){//1
    rect(530, 430, 80, 80, 5)
    if(mouseIsPressed){
      trilha2.stop()
      erro.play()
  ...
  if(mouseX>750 && mouseX<790 && mouseY>450 && mouseY<490){//4
    rect(730, 430, 80, 80, 5)
    if(mouseIsPressed){
      trilha2.stop()
      erro.play()
  ...
  if(mouseX>950 && mouseX<990 && mouseY>450 && mouseY<490){//3
    rect(930, 430, 80, 80, 5)
    if(mouseIsPressed){
      trilha2.stop()
      erro.play()
  ...
  image(pergunta, windowWidth/2-275, windowHeight/2, 500, 70)//pergunta
  image(dois, 350, 450, 40, 40)//1
  image(um, 550, 450, 40, 40)//2
  image(quatro, 750, 450, 40, 40)//3
  image(tres, 950, 450, 40, 40)//4
}
function tela2(){// tela de instruções
  image(volta, 1200, 60, 80, 60)// botão de voltar ao menu
  if(mouseX>1200 && mouseX<1280 && mouseY>60 && mouseY<120){
    if(mouseIsPressed){
      ac_violao.stop()
  ...
  textSize(25)
  text("O objetivo do Jogo é clicar no número de ", windowWidth/5, 240)
  text("instrumentos tocados, de acordo com o som",windowWidth/5, 270)
  text("que estiver sendo executado.", windowWidth/5-8, 300)
  ...
  image(flt, windowWidth/2+250, 350, 79, 79)//flauta
  if(mouseX>windowWidth/2+250 && mouseX<windowWidth/2+250+79 && mouseY>350 && mouseY<350+79){
    if(mouseIsPressed){
      mainmenu.stop()
      fluteSound.play()
      ac_violao.stop()
  ...
  image(pno, windowWidth/2+400, 350, 80, 80)// piano
  if(mouseX>windowWidth/2+400 && mouseX<windowWidth/2+400+80 && mouseY>350 && mouseY<350+80){
    if(mouseIsPressed){
      mainmenu.stop()
      pianoSound.play()
      fluteSound.stop()
...
function tela3(){ // tela de créditos com os nomes dos desenvolvedores
  ...
  textSize(60)
  textAlign(CENTER)
  text("Elaboração e objetivos", windowWidth/2, 70)
  textSize(35)
  ...
  //competências
  textSize(35)
  text("Objetivos", 210, 400)
  textSize(20)
  text("Esse jogo segue a BNCC", 200, 450)
  text("no contexto e práticas do", 200, 480)
  text("ensino de música (EF15AR13)", 200, 510)
